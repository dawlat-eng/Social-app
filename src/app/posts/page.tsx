...
export default function Posts({postdata , allComments=false}:{postdata:PostInterface , allComments?:boolean}) {
 ...
 let  handleSinglePost = (id:string)=>{
  ...
  <Card>
    <CardHeader
      avatar={
        <Avatar  onClick={()=>{handleProfile(postdata?._id)}} sx={{ bgcolor: red[500] }} aria-label="recipe">
          <Image src={postdata?.user?.photo? postdata?.user?.photo : img } 
  ...
  />
    ...
    <CardActions disableSpacing>
      <IconButton onClick={()=>handleSinglePost(postdata?._id)} aria-label="add to favorites">
        <CommentIcon />
      </IconButton>
  ...
  {postdata?.comments.length > 0 && !allComments ? <Box>
  <CardHeader
      avatar={
        <Avatar  onClick={()=>{handleProfile(postdata?.comments[0]?.commentCreator._id)}} 
  ...
  (
    ...
    <CardHeader
        avatar={
          <Avatar  onClick={()=>{handleProfile(comment?.commentCreator?._id)}} 
          sx={{ bgcolor: red[500] }} aria-label="recipe">
